...
STRENGTH > character_inventory_slots_bonus , character_mining_speed_modifier, melee damage
...
VITALITY > character_health_bonus, melee life gain
...
local function update_player_stats(player)
	...
	character.character_inventory_slots_bonus = math.round(strength * 0.2, 3)
	character.character_mining_speed_modifier = math.round(strength * 0.008, 3)
	
	local magic = rpg_t[player.index].magic - 10
	local v = magic * 0.15
	character.character_build_distance_bonus = math.round(v, 3)
	character.character_item_drop_distance_bonus = math.round(v, 3)
	character.character_reach_distance_bonus = math.round(v, 3)
	character.character_loot_pickup_distance_bonus = math.round(v * 0.5, 3)
	character.character_item_pickup_distance_bonus = math.round(v * 0.25, 3)
	character.character_resource_reach_distance_bonus = math.round(v * 0.15, 3)
	
	local dexterity = rpg_t[player.index].dexterity - 10
	character.character_running_speed_modifier = math.round(dexterity * 0.002, 3)
	character.character_crafting_speed_modifier = math.round(dexterity * 0.015, 3)
	
	character.character_health_bonus = math.round((rpg_t[player.index].vitality - 10) * 6, 3)
end
